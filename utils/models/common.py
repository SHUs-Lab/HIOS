...
def placeholder(output_shape, rank = 0, name = None):
    """
    Placeholder.
    ...
    return Value(holder, 0, holder.output_shape[0], holder.output_shape[1], holder.output_shape[2], rank, tag)


def conv2d(block: Block, inputs, out_channels, kernel=(1, 1), stride=(1, 1), padding=(0, 0), groups=1, act="relu",
           is_exit=False, rank=0):
    """
    ...
    name = new_name()
    tag = random.randint(1,30000)
    conv = Conv(name, name, inputs, out_channels, kernel, stride, padding, groups, act, None, rank)
    conv.infer_shape()
    for ti, term in enumerate(inputs):
        for vi, value in enumerate(term):
            value.node.uses.append((conv, ti, vi))
            value.node.outdeg = value.node.outdeg + 1
    if is_exit:
        block.exit_node = conv
    else:
        block.inner_nodes.append(conv)

    return Value(conv, 0, out_channels, conv.output_shape[1], conv.output_shape[2], rank, tag)
...
def identity(block: Block, inputs, rank = 0, is_exit=False):
    """
    ...
    for ti, term in enumerate(inputs):
        for vi, value in enumerate(term):
            ...
            value.node.outdeg = value.node.outdeg + 1
            #value.node.update_childs(name)
    if is_exit:
        block.exit_node = ident
    else:
        block.inner_nodes.append(ident)
    return Value(ident, 0, ident.output_shape[0], ident.output_shape[1], ident.output_shape[2], rank, tag)


def mpi_sr(block: Block, inputs, com_type, rank = 0, is_exit=False):

    name = new_name()
    tag = random.randint(1,30000)
    mpisr = MPI_sr(name, name, inputs, com_type, rank)
    mpisr.infer_shape()
    for ti, term in enumerate(inputs):
        for vi, value in enumerate(term):
            value.node.uses.append((mpisr, ti, vi))

            #mpisr.output_shape = value.node.output_shape
    if is_exit:
        block.exit_node = mpisr
    else:
        block.inner_nodes.append(mpisr)

    return Value(mpisr, 0, mpisr.output_shape[0], mpisr.output_shape[1], mpisr.output_shape[2],rank,tag)



def relu(block: Block, inputs, is_exit=False, rank = 0):
    """
    ...
    return Value(rel, 0, rel.output_shape[0], rel.output_shape[1], rel.output_shape[2], rank, tag)
...
def pool2d(block: Block, inputs, pool_type, kernel=(1, 1), stride=(1, 1), padding=(0, 0), is_exit=False, rank=0):
    """
    ...
    return Value(pool, 0, pool.output_shape[0], pool.output_shape[1], pool.output_shape[2], rank, tag)


def sequential(block: Block, hint_name, nodes: List[Node], is_exit=False, rank = 0):
    """
    ...
    for i in range(len(nodes) - 1):
        ...
        nodes[i].tag = tag
        nodes[i].rank = rank
        nodes[i + 1].inputs = [[Value(nodes[i], 0, nodes[i].output_shape[0],nodes[i].output_shape[1],nodes[i].output_shape[2], rank, tag)]]
    ...
    return Value(seq, 0, seq.output_shape[0], seq.output_shape[1], seq.output_shape[2], rank, tag)
...
def relu_sep_conv(block: Block, inputs: List[List[Value]], out_channels, kernel, stride, padding, is_exit=False, rank=0):
    """
...
def relu_conv(block: Block, inputs, out_channels, kernel=(1, 1), stride=(1, 1), padding=(0, 0), groups=1,
    ...
    names = [new_name(), new_name()]
    nodes = [
        Relu(names[0], names[0], inputs, output_shape=None),
