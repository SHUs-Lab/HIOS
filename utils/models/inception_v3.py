...
def inception_front(v: Value) -> Tuple[Value, Block]:  # 3 x 299 x 299
    ...
    v = pool2d(block, [[v]], pool_type='max', kernel=(3, 3), stride=(2, 2))  # 192 x 35 x 35
    return identity(block, [[v]], is_exit=True), block
...
def inception_logits(v):
    ...
    v = pool2d(block, [[v]], pool_type='global_avg')
    return identity(block, [[v]], is_exit=True), block


def inception_v3(height, width) -> Graph:
    reset_name()
    #pv = placeholder(output_shape=(3, 1536, 1536))
    pv = placeholder(output_shape=(3, height, width))
    ...
    #graph = Graph("inception_v3", pv.node,[block1])
    #graph = Graph("inception_v3", pv.node, [block1,block2])
